...
public class STAXParser implements ErrorHandler, EntityResolver {
    ...
    public STAXJob parse(String xmlSource, String xmlFile, String xmlMachine,
        ...
        try {
            ...
            StringReader transformedXML = (StringReader) fXMLTransformer.transform(new StringReader(xmlSource));
            ...
            for (int i = 0; i < children.getLength(); ++i) {
                ...
                if ((thisChild.getNodeType() == Node.ELEMENT_NODE) && (thisChild.getNodeName().equals("defaultcall"))) {
                    ...
                    STAXActionFactory factory = fSTAX.getActionFactory(thisChild.getNodeName());

                    if (factory == null) {
                        STAXActionDefaultImpl defaultCallAction = new STAXActionDefaultImpl();
                        ...
                        defaultCallAction.setElementInfo(new STAXElementInfo(thisChild.getNodeName(),
                    ...
                    STAXAction action = factory.parseAction(fSTAX, job, thisChild);
                    // STAX element only include function/signalhandler/script elements
            ...
            if (fSTAX.getDebugXmlParser()) {
                STAX.logToJVMLog("Debug", jobID, "STAXParser::parse: Step 4: Call parseActions: " + (System.currentTimeMillis() - start));
            }
        } catch (IOException e) {
        ...
        try {
            ...
            for (int i = 0; i < children.getLength(); ++i) {
                ...
                if ((thisChild.getNodeType() == Node.ELEMENT_NODE) &&
                    ...
                    if (factory == null) {
                        throw new STAXInvalidXMLElementException(thisChild.getNodeName());
                    }
    ...
    private void handleDefaultFunction(STAXJob job, Node root) throws STAXException {
        NamedNodeMap attrs = root.getAttributes();
        ...
        for (int i = 0; i < children.getLength(); ++i) {
            ...
            if (thisChild.getNodeType() == Node.COMMENT_NODE) {
                ...
                defaultCallAction.setXmlFile(job.getXmlFile());
                defaultCallAction.setXmlMachine(job.getXmlMachine());
                defaultCallAction.setElementInfo(new STAXElementInfo("defaultcall"));

                job.setStartFuncArgs(STAXUtil.parseAndCompileForPython(thisChild.getNodeValue(), defaultCallAction));
