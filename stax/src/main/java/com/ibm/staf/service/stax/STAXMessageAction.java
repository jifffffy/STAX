...
public class STAXMessageAction extends STAXActionDefaultImpl {
    public STAXMessageAction() { /* Do Nothing */ }
    ...
    public void execute(STAXThread thread) {
        ...
        try {
            ...
            if (!fIf) {   // Ignore message element if "if" attribute evaluates to FALSE
                fThread.popAction();
                return;
            }
        ...
        messageMap.put("messagetext", timestamp.getTimestampString() + " " + STAFUtil.maskPrivateData(fMessageValue));

        fThread.getJob().generateEvent(STAXMessageActionFactory.STAX_MESSAGE, messageMap);

        if (fLogAttr) {
            // Log the message in the STAX Job User log

            STAFResult result = fThread.getJob().log(STAXJob.USER_JOB_LOG, fLevel, fMessageValue);

            if ((result.rc == 4004) && (fThread.getJob().getInvalidLogLevelAction() == STAXLogAction.LOGINFO)) {
                // Invalid log level.  Log using "Info" for the log level.
                result = fThread.getJob().log(STAXJob.USER_JOB_LOG, "Info", fMessageValue);
            }

            if ((result.rc != STAFResult.Ok) && (result.rc != STAFResult.UnknownService)) {
                fThread.popAction();
                ...
                setElementInfo(new STAXElementInfo(evalElem, STAXElementInfo.NO_ATTRIBUTE_NAME, msg));

                fThread.setSignalMsgVar("STAXLogMsg", STAXUtil.formatErrorMessage(this));
