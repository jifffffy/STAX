...
public class STAXFunctionAction extends STAXActionDefaultImpl {
    ...
    public STAXFunctionAction() { /* Do Nothing */ }
    ...
    public int getArgDefinition() {
        return fArgDefinition;
    }

    public void setArgDefinition(int def) {
        fArgDefinition = def;
    }
    ...
    public void addArg(STAXFunctionArgument arg) {
        fArgList.add(arg);
    }

    public PyObject getCallArgs() {
        return fCallArgPyObject;
    }

    public void setCallArgs(PyObject args) {
        fCallArgPyObject = args;
    }

    public void setCallAction(STAXCallAction action) {
        fCallAction = action;
    }
    ...
    public List<STAXFunctionImport> getImportList() {
        return fImportList;
    }
    ...
    public void execute(STAXThread thread) {
        if (fState == INIT) {
            ...
            if (verifyArguments(thread, signalData) == 0) {
                ...
                functionBeginMap.put("prolog", fProlog);
                functionBeginMap.put("epilog", fEpilog);
            ...
            functionEndMap.put("prolog", fProlog);
            functionEndMap.put("epilog", fEpilog);
    ...
    public int verifyArguments(STAXThread thread, STAXSignalData signalData) {
        if (fArgDefinition == FUNCTION_DEFINES_NO_ARGS) {
            ...
            if (fCallArgPyObject != null && !(fCallArgPyObject.toString().equals("None"))) {
            ...
            for (int i = 0; i < fArgList.size(); i++) {
                ...
                if (argType == ARG_REQUIRED) {
                    ...
                    for (Map.Entry<Object, PyObject> entry : callArgMap.entrySet()) {
                        m.put(Py.java2py(entry.getKey()), Py.java2py(entry.getValue()));
                    }

                    callArgMap = new HashMap<Object, PyObject>(); // Reset to empty

                    // PyDictionary constructor wants a Hashtable
                    thread.pySetVar(argName, new PyDictionary(new Hashtable<PyObject, PyObject>(m)));
                } else {   // Should never happen
                    STAX.logToJVMLog(
                            "Error", thread,
                            "STAXFunctionAction.verifyArguments: " +
