...
public class STAXCallAction extends STAXActionDefaultImpl {
    ...
    public STAXCallAction() { /* Do Nothing */ }
        ...
        try {
            if (fCallType == CALL_ONE_ARG) {
                ...
                for (int i = 0; i < fArgList.size(); i++) {
                    ...
                    ((PyList) args).append(Py.java2py(thread.pyObjectEval(value)));
                }
            } else if (fCallType == CALL_MAP_ARGS) {
                // Evaluate fArgMap data and put in a Python dictionary object

                Map<PyObject, PyObject> evalArgMap = new HashMap<PyObject, PyObject>();
                String key;
                String value;
                ...
                args = new PyDictionary(new Hashtable<PyObject, PyObject>(evalArgMap));
            } else {   // Should never happen
                STAX.logToJVMLog("Error", thread, "STAXCallAction::execute() - Invalid call type=" + fCallType);
                return;
            }
        } catch (STAXPythonEvaluationException e) {
