...
public class STAXSTAFCommandAction extends STAXActionDefaultImpl implements STAXSTAFRequestCompleteListener {
    ...
    // 异步线程再次执行这个action的时候fState = COMMAND_COMPLETE
        ...
        if (fState == INIT) {
            ...
            try {
                ...
                if (!fName.equals("")) {
                    ...
                    fName = "STAFCommand" + thread.getJob().getNextCmdNumber();
                    fUnevalName = fName;
                }
                ...
                fThread.pySetVar("STAFResult", "STAF command failed to start.  " + "Raised a STAXPythonEvaluationError signal.");
            ...
            STAFResult submitResult = fThread.getJob().submitAsync(fLocation, fService, fRequest, this);
            ...
            //TODO 添加fHoldCondition, 本次action执行完后, action依然在fThread的actionStack种,
            // 因此action执行完后,会再次执行这个action, 由于添加了fHoldCondition, fThread会进入BLOCK状态,并退出STAXThread这次执行, STAXThreadQueue可以去执行其他的STAXThread,
            // 当requestComplete调用的时候,移除fHoldCondition,再次执行这个action
            ...
            TreeMap<String, STAXSTAFCommandAction> stafcmds = (TreeMap<String, STAXSTAFCommandAction>) fThread.getJob().getData("stafcmdRequestMap");
    ...
    public synchronized void handleCondition(STAXThread thread, STAXCondition cond) {
        ...
        if (fRequestNumber != 0) {
            ...
            TreeMap stafcmds = (TreeMap) thread.getJob().getData("stafcmdRequestMap");
    ...
    public synchronized void requestComplete(int requestNumber, STAFResult result) {
        ...
        try {
            ...
            setElementInfo(new STAXElementInfo(getElement(), STAXElementInfo.NO_ATTRIBUTE_NAME, "Error while unmarshalling the STAFCommand result"));
    ...
    protected String fUnevalLocation = "";
    protected String fUnevalService = "";
    protected String fUnevalRequest = "";
    protected String fUnevalName = "";
