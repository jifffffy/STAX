...
public class STAXBlockAction extends STAXActionDefaultImpl implements STAXTimedEventListener {
    ...
    public void holdBlock(long timeout) {
        ...
        if (timeout != 0) {
            fHoldTimedEvent = new STAXTimedEvent(System.currentTimeMillis() + timeout, this);

            fOwningThread.getJob().getTimedEventQueue().addTimedEvent(fHoldTimedEvent);
        ...
        fOwningThread.getJob().generateEvent(STAXBlockActionFactory.STAX_BLOCK_EVENT, blockHoldMap);
    ...
    public void releaseBlock() {
        ...
        HashMap<String, String> blockReleaseMap = new HashMap<String, String>();
        ...
        fOwningThread.getJob().generateEvent(STAXBlockActionFactory.STAX_BLOCK_EVENT, blockReleaseMap);

        if (fHoldTimedEvent != null) {
            fOwningThread.getJob().getTimedEventQueue().removeTimedEvent(fHoldTimedEvent);
    ...
    public void terminateBlock() {
        ...
        HashMap<String, String> blockTermMap = new HashMap<>();
        blockTermMap.put("type", "block");
        blockTermMap.put("block", fName);
        blockTermMap.put("status", "terminate");
        blockTermMap.put("name", fName);

        fOwningThread.getJob().generateEvent(STAXBlockActionFactory.STAX_BLOCK_EVENT, blockTermMap);
            ...
            case INIT:
                ...
                if (!parentBlockName.equals("None")) {
                    try {
                        ...
                        fName = parentBlockName + "." + thread.pyStringEval(fUnevalName);
                    } catch (STAXPythonEvaluationException e) {
                        fState = COMPLETE;
                        thread.popAction();

                        thread.pySetVar("STAXBlockRC", STAXJob.ABNORMAL_STATUS);
                        ...
                        // signal action should be executed
                ...
                TreeMap<String, STAXBlockAction> blockMap = (TreeMap<String, STAXBlockAction>) thread.getJob().getData("blockMap");
                // add the fName to the current job block map
                if (blockMap != null) {
                    ...
                    if (!valid_blockName) {
                        ...
                        thread.setSignalMsgVar("STAXInvalidBlockNameMsg", STAXUtil.formatErrorMessage(this));
                ...
                try {
                    ...
                    thread.getJob().log(STAXJob.JOB_LOG, "error", "STAXBlockAction: Enter block " + fName + " failed with " + e.toString());
                    return;
                }

                HashMap<String, String> blockBeginMap = new HashMap<>();
                blockBeginMap.put("type", "block");
                blockBeginMap.put("block", fName);
                blockBeginMap.put("status", "begin");
                blockBeginMap.put("name", fName);

                thread.getJob().generateEvent(STAXBlockActionFactory.STAX_BLOCK_EVENT, blockBeginMap);
                ...
                break;
            case ACTION_CALLED:
                HashMap<String, String> blockEndMap = new HashMap<String, String>();
                blockEndMap.put("type", "block");
                blockEndMap.put("block", fName);
                blockEndMap.put("status", "end");
                blockEndMap.put("name", fName);

                thread.getJob().generateEvent(STAXBlockActionFactory.STAX_BLOCK_EVENT, blockEndMap);

                fState = COMPLETE;

                thread.pySetVar("STAXBlockRC", STAXJob.NORMAL_STATUS);
    ...
    public void exitBlock(String name, STAXThread thread) {
        try {
            if (thread.pyBoolEval("STAXBlockStack[len(STAXBlockStack)-1] != " + "STAXCurrentBlock")) {
                thread.getJob().log(STAXJob.JOB_LOG, "error", "STAXBlockAction: Exit Block " + name + " failed.  This block is not on the STAXBlockStack");
            } else {
                thread.pyExec("STAXBlockStack.pop()");
                if (thread.pyBoolEval("len(STAXBlockStack) > 0"))
                    thread.pyExec("STAXCurrentBlock = " + "STAXBlockStack[len(STAXBlockStack)-1]");
                else
                    thread.pySetVar("STAXCurrentBlock", Py.None);
            }
        } catch (STAXPythonEvaluationException e) {
            thread.getJob().log(STAXJob.JOB_LOG, "error", "STAXBlockAction: Exit Block(" + name + " failed with " + e.toString());
        }
    ...
    public void handleCondition(STAXThread thread, STAXCondition cond) {
        ...
        if (cond instanceof STAXTerminateBlockCondition) {
            ...
            thread.pySetVar("STAXBlockRC", STAXJob.TERMINATED_STATUS);
        } else {
            thread.pySetVar("STAXBlockRC", STAXJob.NORMAL_STATUS);
        }
    ...
    private final STAXHoldThreadCondition fHoldCondition = new STAXHoldThreadCondition("Block");
    private final STAXTerminateBlockCondition fTermCondition = new STAXTerminateBlockCondition("Block");
