...
public class STAXProcessAction extends STAXActionDefaultImpl
    ...
    private STAXHoldThreadCondition fHoldCondition = new STAXHoldThreadCondition("Process");
    private STAXHardHoldThreadCondition fHardHoldCondition = new STAXHardHoldThreadCondition("Process");
    ...
    private HashMap<Integer, STAXThread> fThreadMap = new HashMap<Integer, STAXThread>();
    ...
    public STAXProcessAction() { /* Do Nothing */ }

    public STAXProcessActionFactory getActionFactory() {
        return fFactory;
    }
    ...
    public String getUseprocessvars() {
        return fUseprocessvars;
    }
    ...
    public String getDisabledauth() {
        return fDisabledauth;
    }
    ...
    public String getStatichandlename() {
        return fStatichandlename;
    }

    public void setStatichandlename(String statichandlename, String statichandlenameIf) {
    ...
    public Vector<String> getReturnFiles() {
        return fReturnFiles;
    }
    ...
    public STAXAction getProcessAction() {
        return fProcessAction;
    }

    public void setProcessAction(STAXAction processAction, String processActionIf) {
        ...
        synchronized (this) {
            if (fState == INIT) {
                ...
                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + " STAXProcessAction::execute(): fState=INIT");
                }
                ...
                if (generateProcessStartRequest() != 0) {
                    if (fThread.getJob().getSTAX().getDebugProcess()) {
                        STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + " STAXProcessAction::execute(): " + "generateProcessStartRequest() failed - return");
                    }
                ...
                fProcessKey = Integer.toString(fThread.getJob().getNextProcessKey());

                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + " STAXProcessAction::execute(): " + "Submitting PROCESS START NOTIFY ONEND KEY " + fProcessKey + " " + fRequest);
                }

                // Submit PROCESS START STAF Command

                STAFResult submitResult = fThread.getJob().submitAsync(fLocation, "process", "START NOTIFY ONEND KEY " + fProcessKey + " " + fRequest, this);

                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + " STAXProcessAction::execute(): " + "PROCESS START request submit RC=" + submitResult.rc + ", Result=" + submitResult.result);
                }
                ...
                if (fRequestNumber == 0) {
                    ...
                    setPythonVariablesWhenFail("Process failed to start.  Raised a " + "STAXProcessStartError signal.");

                    setElementInfo(new STAXElementInfo(getElement(), STAXElementInfo.NO_ATTRIBUTE_NAME, "The process failed to start, Request#: 0, RC: " + submitResult.rc + ", STAFResult: " + submitResult.result));

                    fThread.setSignalMsgVar("STAXProcessStartErrorMsg", STAXUtil.formatErrorMessage(this));
                ...
                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + " STAXProcessAction::execute(): " + "Add timed event - timeout=" + fThread.getJob().getSTAX().getProcessTimeout());
                }

                // Add a TimedEvent to the queue and return 

                fTimedEvent = new STAXTimedEvent(System.currentTimeMillis() + fThread.getJob().getSTAX().getProcessTimeout(), this);

                fThread.getJob().getTimedEventQueue().addTimedEvent(fTimedEvent);

                fThread.addCondition(fHoldCondition);

                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + " STAXProcessAction::execute(): " + "Added hold condition: State=" + getStateAsString());
                }
            } else if (fState == REQUEST_ERROR) {
                fState = COMPLETE;
                fThread.popAction();

                fThread.pySetVar("RC", fRequestRC);
                fThread.pySetVar("STAFResult", fRequestResult);
                fThread.pySetVar("STAXResult", Py.None);

                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + ":" + fProcessHandle + " STAXProcessAction::execute(): " + "fState=REQUEST_ERROR - " + "Raise STAXProcessStartError signal");
                }

                setElementInfo(new STAXElementInfo(getElement(), STAXElementInfo.NO_ATTRIBUTE_NAME, "The process failed to start, RC: " + fRequestRC + ", STAFResult: " + fRequestResult));
                ...
                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + ":" + fProcessHandle + " STAXProcessAction::execute(): " + "fState=REQUEST_TIMEOUT - Raise " + "STAXProcessStartTimeout signal");
                }

                String timeoutMsg = "Process did not start within timeout " + "value " + fThread.getJob().getSTAX().getProcessTimeout();

                // Set RC, STAFResult, STAXResult to indicate a timeout signal

                fRequestRC = STAFResult.Timeout;
                fThread.pySetVar("RC", STAFResult.Timeout);
                fThread.pySetVar("STAFResult", timeoutMsg + ".  Raised a STAXProcessStartTimeout signal");
                fThread.pySetVar("STAXResult", Py.None);

                setElementInfo(new STAXElementInfo(getElement(), STAXElementInfo.NO_ATTRIBUTE_NAME, timeoutMsg));

                fThread.setSignalMsgVar("STAXProcessStartTimeoutMsg", STAXUtil.formatErrorMessage(this));

                fThread.raiseSignal("STAXProcessStartTimeout");
            } else if ((fState == RUNNING) && !fIsProcessRunning && !fIsProcessActionRunning) {
                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + ":" + fProcessHandle + " STAXProcessAction::execute(): fState=" + getStateAsString());
                }
                ...
                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + ":" + fProcessHandle + " STAXProcessAction::execute(): fState=COMPLETE, " + "Set RC=" + fProcessRC + ", STAFResult=" + fProcessSTAFResult);
                    // ", STAXResult=" + fProcessSTAXResult.length());
                }

                fThread.pySetVar("RC", fProcessRC);
                ...
                try {
                    ...
                    setElementInfo(new STAXElementInfo(getElement(), STAXElementInfo.NO_ATTRIBUTE_NAME, "Error setting STAXResult to resultList.\n" + "fileList: " + fProcessSTAXResult.toString() + "\n\n" + ex.toString()));

                    fThread.setSignalMsgVar("STAXPythonEvalMsg", STAXUtil.formatErrorMessage(this));
                ...
                if (fThread.getJob().getSTAX().getDebugProcess()) {
                    STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + ":" + fProcessHandle + " STAXProcessAction::execute(): fState=COMPLETE " + " when shouldn't be called in this state");
                }
                ...
                STAX.logToJVMLog("Debug", fThread, "key=" + fLocation + ":" + fProcessHandle + " STAXProcessAction::execute(): fState=" + getStateAsString() + " is an unexpected state");
            }
    ...
    public synchronized void handleCondition(STAXThread thread, STAXCondition cond) {
        synchronized (this) {
            ...
            if ((fState == RUNNING) && fIsProcessRunning) {
                ...
                thread.getJob().submitAsyncForget(fLocation, "PROCESS", "START SHELL COMMAND " + STAFUtil.wrapData(stopFreeProcessCommand) + " WAIT");
                ...
                TreeMap processes = (TreeMap) thread.getJob().getData("processRequestMap");
            ...
            synchronized (fThreadMap) {
                if (!fThreadMap.isEmpty()) {
                    ...
                    for (STAXThread childThread : fThreadMap.values()) {
                        childThread.terminate(STAXThread.THREAD_END_STOPPED_BY_PARENT);
                    }
    ...
    public synchronized void requestComplete(int requestNumber, STAFResult result) {
        ...
        if (result.rc == 0) {
            ...
            try {
                ...
                HashMap<String, Object> processMap = (HashMap<String, Object>) fThread.getJob().getData("processMap");
                ...
                if (processMap != null) {
                    synchronized (processMap) {
                        ...
                        if (processMap.containsKey(key)) {
                            ...
                            STAXSTAFMessage completionMessage = (STAXSTAFMessage) processMap.get(key);
                            ...
                            if ((fProcessAction != null) && fProcessActionFlag) {
                                ...
                                synchronized (fThreadMap) {
                                    ...
                                    fThread.pySetVar("STAXProcessHandle", String.valueOf(fProcessHandle));
            ...
            if (fIsProcessRunning) {
                ...
                @SuppressWarnings("unchecked")
                TreeMap<String, STAXProcessAction> processes = (TreeMap<String, STAXProcessAction>) fThread.getJob().getData("processRequestMap");
            ...
            if ((result.rc == STAFResult.InvalidRequestString) &&
                ...
                STAFResult submitResult = fThread.getJob().submitAsync(fLocation, "PROCESS", "START NOTIFY ONEND " + fRequest, this);
                ...
                if (fRequestNumber == 0) {
                    ...
                    fThread.pySetVar("RC", submitResult.rc);
    ...
    public synchronized void processComplete(String machine, int handle, long rc, List fileList, String timestamp) {
        if (fThread.getJob().getSTAX().getDebugProcess()) {
            STAX.logToJVMLog(
                    "Debug", fThread,
                    "key=" + fLocation + ":" + fProcessHandle +
        ...
        fThread.getJob().submitAsyncForget(fLocation, "PROCESS", "FREE HANDLE " + fProcessHandle);
        ...
        if (fIsProcessRunning) {
            ...
            TreeMap processes = (TreeMap) fThread.getJob().getData("processRequestMap");
    ...
    public PyList getResultList(List fileList) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                // Assign file RC
                fileInfoList.append(Py.java2py(new Integer((String) fileInfoMap.get("rc"))));
                fileInfoList.append(Py.java2py(fileInfoMap.get("data")));
    ...
    private int generateProcessStartRequest() {
        ...
        try {
            ...
            if (!fName.equals("")) {
                ...
                fName = "Process" + fThread.getJob().getNextProcNumber();
                fUnevalName = fName;
            }
            ...
            if (!fCommandMode.equals("")) {
                ...
                if (!(fCommandMode.equalsIgnoreCase("shell")) && !(fCommandMode.equalsIgnoreCase("default"))) {
                    ...
                    setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex, "Invalid value: " + fCommandMode + "\n\nValid values for attribute \"mode\" are: " + "'default' or 'shell'."));
            ...
            for (int i = 0; i < fUnevalVars.size(); i++) {
                ...
                if (fThread.pyBoolEval(varsIf)) {
                    ...
                    evalAttr = STAXElementInfo.NO_ATTRIBUTE_NAME;
                    List varList = fThread.pyListEval(fUnevalVars.elementAt(i));
            ...
            for (int i = 0; i < fUnevalEnvs.size(); i++) {
                ...
                if (fThread.pyBoolEval(envsIf)) {
                    // Evaluate as a list; each item in list is a single var
                    evalAttr = STAXElementInfo.NO_ATTRIBUTE_NAME;
                    List envList = fThread.pyListEval(fUnevalEnvs.elementAt(i));
            ...
            if (!fConsole.equals("")) {
                ...
                if (fThread.pyBoolEval(fConsoleIf)) {
                    ...
                    if (fConsole.equalsIgnoreCase("new"))
                        ...
                        setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex, "Invalid value: " + fConsole + "\n\nValid values for attribute \"use\" are: " + "'new' or 'same'."));
            ...
            if (!fFocus.equals("")) {
                ...
                if (fThread.pyBoolEval(fFocusIf)) {
                    ...
                    if (fFocus.equalsIgnoreCase("minimized")) {
                        ...
                        setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex, "Invalid valid: " + fFocus + "\n\nValid values for attribute \"mode\" are: " + "'minimized', 'foreground', or 'background'."));
            ...
            if (!fDisabledauth.equals("")) {
                ...
                if (fThread.pyBoolEval(fDisabledauthIf)) {
                    ...
                    if (fDisabledauth.equalsIgnoreCase("error"))
                        ...
                        setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex, "Invalid value: " + fDisabledauth + "\n\nValid values for attribute \"action\" are: " + "'error' or 'ignore'."));
            ...
            if (!fStatichandlename.equals("")) {
                ...
                if (fThread.pyBoolEval(fStatichandlenameIf)) {
                    evalAttr = STAXElementInfo.NO_ATTRIBUTE_NAME;
                    fStatichandlename = fThread.pyStringEval(fUnevalStatichandlename);
            ...
            if (!fStdout.equals("")) {
                ...
                if (fThread.pyBoolEval(fStdoutIf)) {
                    ...
                    if (fStdoutMode.equalsIgnoreCase("replace")) {
                        ...
                        setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex, "Invalid value: " + fStdoutMode + "\n\nValid values for attribute \"mode\" are: " + "'replace' or 'append'."));
            ...
            if (!fStderr.equals("") || !fUnevalStderrMode.equals("")) {
                ...
                if (fThread.pyBoolEval(fStderrIf)) {
                    ...
                    if (fStderrMode.equalsIgnoreCase("replace")) {
                        ...
                        setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex, "Invalid value: " + fStderrMode + "\n\nValid values for attribute \"mode\" are: " + "'replace', 'append', or 'stdout'."));
            ...
            for (int i = 0; i < fUnevalReturnFiles.size(); i++) {
                evalIndex = i;
                evalAttr = "if";
                String returnFilesIf = fThread.pyStringEval(fReturnFilesIf.elementAt(i));

                if (fThread.pyBoolEval(returnFilesIf)) {
                    // Evaluate as a list; each list item is a single file

                    evalAttr = STAXElementInfo.NO_ATTRIBUTE_NAME;
                    List fileList = fThread.pyListEval(fUnevalReturnFiles.elementAt(i));
            ...
            setPythonVariablesWhenFail("Process failed to start.  Raised a " + "STAXPythonEvaluationError signal.");

            setElementInfo(new STAXElementInfo(evalElem, evalAttr, evalIndex));

            fThread.setSignalMsgVar("STAXPythonEvalMsg", STAXUtil.formatErrorMessage(this), e);
    ...
    private void generateProcessStartEvent() {
        ...
        processMap.put("envList", STAFMarshallingContext.marshall(envList, new STAFMarshallingContext()));
