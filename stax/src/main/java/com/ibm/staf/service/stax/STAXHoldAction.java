...
public class STAXHoldAction extends STAXActionDefaultImpl {
    public STAXHoldAction() { /* Do Nothing */ }
    ...
    public void execute(STAXThread thread) {
        ...
        try {
            ...
            if (fUnevalTimeoutString != null) {
                ...
                STAFResult result = STAFUtil.convertDurationString(fTimeoutString);
                ...
                if (result.rc != STAFResult.Ok) {
                    throwSignal(thread,
                            new STAXElementInfo(evalElem, evalAttr, "Invalid timeout value: " + fTimeoutString + "\n\n" + result.result),
                            "STAXInvalidTimerValueMsg",
                            "STAXInvalidTimerValue",
                            null);
                    return;
                }
            }
        } catch (STAXPythonEvaluationException e) {
            throwSignal(thread, new STAXElementInfo(evalElem, evalAttr), "STAXPythonEvalMsg", "STAXPythonEvaluationError", e);
            return;
        }
        ...
        if (theBlock == null) {
            setElementInfo(new STAXElementInfo(evalElem, evalAttr, "Block '" + fBlockName + "' does not exist."));

            thread.setSignalMsgVar("STAXBlockDoesNotExistMsg", STAXUtil.formatErrorMessage(this));
