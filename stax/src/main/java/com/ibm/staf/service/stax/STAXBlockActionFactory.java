...
public class STAXBlockActionFactory implements STAXActionFactory, STAXListRequestHandler, STAXQueryRequestHandler, STAXGenericRequestHandler, STAXJobManagementHandler {
    static final String STAX_BLOCK_EVENT = "Block";

    static final String sBlockInfoMapClassName = "STAF/Service/STAX/BlockInfo";
    static final String sQueryBlockMapClassName = "STAF/Service/STAX/QueryBlock";
    ...
    public STAXBlockActionFactory(STAX staxService) {
        ...
        try {
            ...
            int rc = staxService.registerGenericRequestHandler(this, serviceInterfaceClass);
            if (rc != 0) {
                STAX.logToJVMLog("Error", "STAXBlockActionFactory: " + "registerGenericRequestHandler() failed");
            }
        } catch (ClassNotFoundException e) {
            STAX.logToJVMLog("Error", "STAXBlockActionFactory: " + "registerGenericRequestHandler: " + e);
        }
    ...
    public void initJob(STAXJob job) {
        if (!job.setData("blockMap", new TreeMap<String, STAXBlockAction>())) {
            String msg = "STAXBlockActionFactory.initJob: setData for " + "blockMap failed.";
    ...
    public STAFResult handleListRequest(String type, STAXJob job, STAXRequestSettings settings) {
        if (type.equalsIgnoreCase("blocks")) {
            ...
            List<Map<String, Object>> blockOutputList = new ArrayList<Map<String, Object>>();
            ...
            if (blockMap != null) {
                synchronized (blockMap) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        Map<String, Object> blockOutputMap = new TreeMap<String, Object>();
                        blockOutputMap.put("staf-map-class-name", fBlockInfoMapClass.name());
        ...
        if (type.equalsIgnoreCase("block")) {
            ...
            synchronized (blockMap) {
                if (!blockMap.containsKey(key))
                    ...
                    Map<String, Object> blockOutputMap = new TreeMap<String, Object>();
                    blockOutputMap.put("staf-map-class-name", fQueryBlockMapClass.name());
                    blockOutputMap.put("blockName", block.getName());
                    blockOutputMap.put("state", block.getBlockStateAsString());
                    blockOutputMap.put("threadID", "" + block.getOwningThread().getThreadNumber());
                    blockOutputMap.put("startTimestamp", block.getStartTimestamp().getTimestampString());
    ...
    public STAFResult handleQueryJobRequest(STAXJob job, STAXRequestSettings settings) {
        ...
        if (blockMap != null) {
            synchronized (blockMap) {
                ...
                for (Object o : blockMap.values()) {
                    STAXBlockAction block = (STAXBlockAction) o;

                    if (block.getBlockState() == STAXBlockAction.BLOCK_RUNNING)
                        blocksRunningState += 1;
                    else if (block.getBlockState() == STAXBlockAction.BLOCK_HELD)
                        blocksHeldState += 1;
                    else
    ...
    public STAXAction parseAction(STAX staxService, STAXJob job, Node root) throws STAXException {
        ...
        for (int i = 0; i < attrs.getLength(); ++i) {
            ...
            action.setElementInfo(new STAXElementInfo(root.getNodeName(), thisAttr.getNodeName()));
                    ...
                    if (factory == null) {
                        action.setElementInfo(
                                new STAXElementInfo(
                                        ...
                                        STAXElementInfo.NO_ATTRIBUTE_NAME,
                                        STAXElementInfo.LAST_ELEMENT_INDEX,
                                        "No action factory for element type \"" + thisChild.getNodeName() + "\""));

                        throw new STAXInvalidXMLElementException(STAXUtil.formatErrorMessage(action), action);
                    }

                    blockAction = factory.parseAction(staxService, job, thisChild);
                    break;
                default:
                    action.setElementInfo(
                            new STAXElementInfo(
    ...
    public STAFResult handleRequest(Object infoObject, STAX staxService) {
        ...
        if (parseResult.rc != STAFResult.Ok) {
            ...
            } else {   // Returning nothing in the result indicates that this parser
