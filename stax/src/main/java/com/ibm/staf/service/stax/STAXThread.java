...
public class STAXThread implements STAXThreadCompleteListener {
    ...
    */
    ...
    public STAXThread(STAXJob job) {
        ...
        // add XFS service like block(eg. siu.cim, siu.ptr)
        fPythonInterpreter.exec("XFSServiceStack = []");
    ...
    public void raiseSignal(String name, STAXActionDefaultImpl raiserAction) {
        ...
        // TODO: Should also check if same signalhandler action
        if (fSignalStack.contains(name)) {
            getJob().log(STAXJob.JOB_LOG, "error", "Duplicate signal " + name + ".  Terminating job.");
        ...
        if (handlerAction == null) {
            if (name != "STAXNoSuchSignalHandler") {
                ...
                if (raiserAction != null) {
                    raiserAction.setElementInfo(new STAXElementInfo(raiserAction.getElement(), "signal", msg));

                    setSignalMsgVar("STAXNoSuchSignalHandlerMsg", STAXUtil.formatErrorMessage(raiserAction));
                } else {
     ...
     *                         raised
     */
    public void setSignalMsgVar(String signalMsgVarName, String xmlInfo, STAXPythonEvaluationException pythonException) {
        ...
        if (conditionStack.size() > 0) {
            ...
            msg.append(fThreadNumber);
            msg.append(" =====\n\n");
            msg.append(STAFMarshallingContext.formatObject(getConditionStack()));
        }
    ...
    public void pushAction(STAXAction action) {
        synchronized (fActionStack) {
            ...
            if (fBreakpointFirstFunction && (action instanceof STAXFunctionAction)) {
            ...
            if (!pushBreakpoint && !(fJob.breakpointsEmpty())) {
                ...
                if ((action instanceof STAXFunctionAction) && (fJob.isBreakpointFunction(((STAXFunctionAction) action).getName()))) {
                    pushBreakpoint = true;
                }
            ...
            if (pushBreakpoint) {
                if (action instanceof STAXBreakpointAction) {
                    ...
                    if (fBreakpointActionFactory == null) {
                        fBreakpointActionFactory = (STAXBreakpointActionFactory) fJob.getSTAX().getActionFactory("breakpoint");
                    }
                    ...
                    // action stack
                    STAXAction stepBreakpointAction = fBreakpointActionFactory.createStepBreakpoint(action);
    ...
    public void popAction() {
        synchronized (fActionStack) {
            ...
            if ((lastAction instanceof STAXBreakpointAction) && fStepOverCondition) {
    ...
    public void schedule() {
        ...
        if (initState) {
            ...
            if (fParent != null) {
                eventMap.put("parent", String.valueOf(fParent.getThreadNumber()));
            ...
            if (callStack.size() > 0) {
                eventMap.put("detailText", callStack.get(callStack.size() - 1));
            }
    ...
    /**
     *  依次执行每个action
     *  execute each action of action stack of STAXThread
     *  如果直接return 这个STAXThread对象会action持有,否则会被GC, 只有schedule后才会被再次执行!!!
     *  Action执行步骤:
     *  1、判断actionStack是否执行完
     *  2、没执行完，取出最后一个action
     *  3、执行action
     *  Condition的处理:
     *  1、actionStack执行完
     *  主要处理Child STAXThread没有执行完的各种情况
     *  2、actionStack没有执行完
     *  1)如果是STAXHoldThreadCondition和STAXHardHoldThreadCondition,则阻塞
     *  2)处理Condition,不执行action
     */
    public void execute() {
        int numActions = 0;
        ...
        while (true) {
            //TODO numActions 这个本地变量感觉没啥用, schedule执行后, execute方法已经退出了.
            // execute再次执行是由STAXThreadQueue获取的STAXThread对象,虽然是相同的对象,由于numActions是本地变量,每次执行都会为0,应该提升为成员变量!
            ...
            if (fActionStack.size() == 0) {
                ...
                if (fParent == null) {
                    try {
                        ...
                        fJob.log(STAXJob.JOB_LOG, "error", "Error evaluating" + " STAXBlockRC variable when setting job " + "completion status");
                    }
                }
                ...
                synchronized (fConditionSet) {
                    ...
                    for (STAXCondition cond : fConditionSet) {
                        ...
                        if ((fParent == null) && cond.isInheritable()) {
                            ...
                            String className = STAXUtil.getShortClassName(cond.getClass().getName(), "Condition");

                            StringBuffer errorMsg = new StringBuffer("Unhandled \"");

                            errorMsg.append(className).append("\" condition found at end of job.");

                            if ((cond.getSource() != null) && (cond.getSource().length() > 0)) {
                                errorMsg.append("\nSource: ").append(cond.getSource());

                                if (className.equals("Exception")) {
                                    errorMsg.append("\nInfo: ").append(((STAXExceptionCondition) cond).getData());
                            ...
                            fJob.log(STAXJob.JOB_LOG, "error", errorMsg.toString());
                        } else if (cond.isInheritable()) {
                            // 有parent thread, ParallelAction可以创建child thread, 说明这是一个Parallel child thread
                    ...
                    //TODO 再次执行STAXThread这个对象
                    if (addedParentCondition) fParent.schedule();
                }

                if (fJob.getSTAX().getDebugThread()) {
                    STAX.logToJVMLog("Debug", fJob.getJobNumber(), fThreadNumber, "STAXThread: Complete");
                }
                ...
                while (!fCompletionNotifiees.isEmpty()) {
                    STAXThreadCompleteListener listener = fCompletionNotifiees.removeFirst();
            ...
            synchronized (fConditionSet) {
                ...
                //TODO 如果是STAXHoldThreadCondition和STAXHardHoldThreadCondition
                if ((currCondition != null) && ((currCondition instanceof STAXHoldThreadCondition) || (currCondition instanceof STAXHardHoldThreadCondition))) {
                    if (fJob.getSTAX().getDebugThread()) {
                        String condName = currCondition.getClass().getName();
                        condName = condName.substring(condName.lastIndexOf(".") + 1);

                        STAX.logToJVMLog("Debug", fJob.getJobNumber(), fThreadNumber, "STAXThread: Blocking due to " + condName);
                    }
                    // 改变state, 阻塞STAXThread执行
                    fState = STATE_BLOCKED;
                    return;
                }
            }
            ...
            if (currCondition == null) {
                if (fJob.getSTAX().getDebugThread()) {
                    ...
                    STAX.logToJVMLog("Debug", fJob.getJobNumber(), fThreadNumber, "STAXThread: Calling " + actionName);
                }
                ...
                if (fJob.getSTAX().getDebugThread()) {
                    ...
                    STAX.logToJVMLog("Debug", fJob.getJobNumber(), fThreadNumber, "STAXThread: Calling " + actionName + ", handling " + condName);
                }
                // 处理Condition
    ...
    private TreeSet<STAXCondition> fConditionSet = new TreeSet<STAXCondition>(new STAXConditionComparator());
