...
public class STAXJob implements STAXThreadCompleteListener,
    ...
    public void startExecution() throws STAFException, STAXException {
        ...
        if (fSTAX.getMaxReturnFileSize() != 0) {
            String request = "SET VAR " + STAFUtil.wrapData("STAF/MaxReturnFileSize=" + fSTAX.getMaxReturnFileSize());
        ...
        STAXAction action = fDocument.getFunction(fDocument.getStartFunction());
        ...
        if (action == null) {
            ...
            if (fDefaultCallAction != null) {
                ...
                STAXCallAction callAction = new STAXCallAction(startFunctionUneval, fDocument.getStartFunctionArgs());
                callAction.setLineNumber("<External>", "<Error in ARGS option>");
        ...
        if (fExecuteAndHold != null) {
            if (fExecuteAndHold.length() == 0) {
                ...
                actionList.add(new STAXHoldAction("'main'", "1", fExecuteAndHold));
            }
        }
        ...
        thread.pySetVar("STAXJobStartFunctionName", fDocument.getStartFunction());
        thread.pySetVar("STAXJobStartFunctionArgs", fDocument.getStartFunctionArgs());
        ...
        thread.pySetVar("STAXServiceMachineNickname", fSTAX.getLocalMachineNickname());
        thread.pySetVar("STAXEventServiceName", fSTAX.getEventServiceName());
        thread.pySetVar("STAXEventServiceMachine", fSTAX.getEventServiceMachine());
        thread.pySetVar("STAXServicePath", fSTAX.getServicePath());

        thread.pySetVar("STAXJobUserLog",
                new STAFLog(STAFLog.MACHINE,
                        fSTAX.getServiceName().toUpperCase() + "_Job_" + fJobNumber + "_User",
                        fHandle,
                        0)
        );
        thread.pySetVar("STAXJobLogName", fSTAX.getServiceName().toUpperCase() + "_Job_" + fJobNumber);
        ...
        thread.pySetVar("STAXPythonOutput", STAXPythonOutput.getPythonOutputAsString(getPythonOutput()));

        thread.pySetVar("STAXPythonLogLevel", getPythonLogLevel());

        thread.pySetVar("STAXInvalidLogLevelAction", STAXLogAction.getInvalidLogLevelActionAsString(getInvalidLogLevelAction()));
        ...
        STAXActionDefaultImpl mainBlock = new STAXBlockAction("main", mainSequence);
        ...
        jobRunningMap.put("startTimestamp", fStartTimestamp.getTimestampString());
    ...
    public STAFResult log(int logfile, String level, String message) {
        ...
        String logRequest = "LOG MACHINE LOGNAME " + STAFUtil.wrapData(logName) + " LEVEL " + level;
    ...
    public STAFResult submitAsync(String location, String service, String request, STAXSTAFRequestCompleteListener listener) {
        STAFResult result;

        synchronized (fRequestMap) {
            result = fHandle.submit2(STAFHandle.ReqQueue, location, service, request);
            ...
            if (result.rc == STAFResult.Ok) {
                try {
                    ...
                    Integer requestNumber = STAXUtil.convertRequestNumber(result.result);

                    fRequestMap.put(requestNumber, listener);

                    result.result = requestNumber.toString();
                } catch (NumberFormatException e) {
                    STAX.logToJVMLog("Error", fJobNumber, 0, "STAXJob::submitAsync - " + e.toString());
    ...
    public STAFResult submitSync(String location, String service, String request) {
        ...
        STAFResult result = theHandle.submit2(STAFHandle.ReqSync, location, service, request);
    ...
    private Map<Integer, STAXThread> fThreadMap = new LinkedHashMap<Integer, STAXThread>();
    ...
    class STAFQueueMonitor extends Thread {
            ...
            for (; ; ) {
                ...
                try {
                    ...
                    if (result == null) {
                        ...
                        logMessage("STAF local QUEUE " + request + " returned null. This may have been caused by " + "running out of memory creating the result.", null);

                        if (numErrors < maxErrors) {
                            continue;
                        } else {
                            logMessage("Exiting this thread after the QUEUE GET " + "request failed " + maxErrors + " consecutive times.", null);
                    ...
                    switch (result.rc) {
                        case STAFResult.Ok:
                            numErrors = 0;
                            break;
                        case STAFResult.HandleDoesNotExist:
                        ...
                        default:
                            ...
                            logMessage("STAF local QUEUE " + request + " failed with RC=" + result.rc + ", Result=" + result.result, null);
                    ...
                    logMessage("Exception getting message(s) off the queue.", t);
                ...
                try {
                    ...
                    mc = STAFMarshallingContext.unmarshall(result.result, STAFMarshallingContext.IGNORE_INDIRECT_OBJECTS);

                    messageList = (List) mc.getRootObject();
                } catch (Throwable t) {
                    // Log an error message and continue

                    logMessage("Exception unmarshalling queued messages. " + "\nMarshalled string: " + result.result, t);
                    continue;
                }
                ...
                while (iter.hasNext()) {
                    ...
                    if (type != null && type.equalsIgnoreCase("STAF/Service/STAX/End")) {
        ...
        public void notifyListeners(STAXSTAFMessage msg) {
            ...
            synchronized (fQueueListenerMap) {
                for (Map.Entry<String, TreeSet<STAXSTAFQueueListener>> entry : fQueueListenerMap.entrySet()) {
                    String msgType = entry.getKey();

                    if ((msgType == null) ||
                            // Messages from 3.x clients that STAX is interested
                            // in will have a type
                            (theType != null && theType.equalsIgnoreCase(msgType)) ||
                            ...
                            (theType == null && msg.getMessage().toUpperCase().startsWith(msgType.toUpperCase()))) {

                        TreeSet<STAXSTAFQueueListener> listenerSet = entry.getValue();
