...
public class DialogActionFactory extends ActionFactorySupport<DialogAction> {
    ...
    @Override
    protected void handleRootAttributes(STAX staxService, STAXJob job, DialogAction action, Node root, NamedNodeMap attrs) throws STAXException {
        Node imageNode = attrs.getNamedItem("image");
        if(imageNode != null) {
            action.setImage(imageNode.getNodeValue());
        }
    }
}
